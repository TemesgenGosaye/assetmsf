...
const welcomeSeed = (): Activity[] => {
  const now = Date.now();
  const mk = (minsAgo: number) => new Date(now - minsAgo * 60000).toISOString();
  return [
    { id: -1, type: "system", message: "Welcome to SAMS — your asset hub is ready", user_name: "System", created_at: mk(4) },
    { id: -2, type: "asset_created", message: "Tip: add your first asset to start tracking", user_name: "System", created_at: mk(90) },
    { id: -3, type: "report", message: "Tip: generate a report anytime from the Reports page", user_name: "System", created_at: mk(240) },
  ];
};
...
export function RecentActivity() {
    ...
    (async () => {
      if (isDemoMode()) {
        try {
          ...
          setItems(data.length ? data : welcomeSeed());
        } catch (e) {
          console.error(e);
  ...
  const recent = useMemo(() => sorted, [sorted]);
  const rows = useMemo(() => {
    if (showAll) return recent;
    return recent.slice(0, 7);
  }, [recent, showAll]);
