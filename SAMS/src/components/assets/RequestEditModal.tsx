            ...
            <section className="space-y-3 rounded-2xl border border-border/60 bg-background/85 p-4 sm:p-6">
              ...
              <div className="space-y-2">
                ...
                <Select value={form.type} onValueChange={v => setField('type', v)}>
                  ...
                  <SelectContent>
                    {itemTypes.filter(Boolean).map(t => (<SelectItem key={t} value={t}>{t}</SelectItem>))}
                  </SelectContent>
                </Select>
              </div>
              <div className="space-y-2">
                ...
                <Select value={form.property} onValueChange={v => setField('property', v)}>
                  ...
                  <SelectContent>
                    {visibleProperties.filter(p => p.id).map(p => (<SelectItem key={p.id} value={p.id}>{p.name}</SelectItem>))}
                  </SelectContent>
                </Select>
              </div>
              <div className="space-y-2">
