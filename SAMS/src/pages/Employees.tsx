...
export default function Employees() {
  ...
  const columns = useMemo((): ColDef<Employee>[] => [
    {
      ...
      cell: e => <span className="font-mono text-xs text-muted-foreground tracking-wide">{e.employee_id || "—"}</span>,
    },
    {
  ...
  const handleBulkDelete = async () => {
    ...
    setEmployees(prev => prev.filter(e => !selectedRows.find(s => s.id === e.id)));
  ...
  const handleBulkImport = async () => {
    ...
    try {
      ...
      result.errors.length ? toast.info(`${result.created} created, ${result.skipped} skipped.`) : toast.success(`${result.created} imported.`);
      setEmployees(await listEmployees({ force: true }));
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Select value={form.department ?? "__none__"} onValueChange={v => setForm({ ...form, department: v === "__none__" ? null : v })}>
                    ...
                    <SelectContent><SelectItem value="__none__">None</SelectItem>{departments.filter(d => d.id).map(d => <SelectItem key={d.id} value={d.id}>{d.name}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
                <div className="space-y-2">
