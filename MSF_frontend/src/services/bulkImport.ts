...
import { listDepartments, CONSTANT_DEPARTMENTS } from "./departments";
...
export async function downloadAssetTemplate(filename = "SAMS_Bulk_Import_Template.xlsx") {
  ...
  try {
    ...
    departments = CONSTANT_DEPARTMENTS.map((d) => d.name); // offline fallback
  }
...
export async function importAssetsFromFile(file: File): Promise<ImportResult> {
  ...
  const existing = await listAssets().catch(() => [] as Asset[]);
