...
function buildReceiptHTML(r: HandoverReceipt): string {
  ...
  const handoverDate = fmtDate(r.allocation_date || r.created_at);
  const condition = r.inspection_structural || "Good";
  const notes = r.inspection_admin || "None";
  const inspector = r.generated_by_name || "Authorized Inspector";
...
function buildClearanceSlipHTML(t: TerminationTransaction): string {
  ...
  const damageCosts = fmtCurrency(t.damage_costs);
  const outstandingIssues = t.outstanding_issues || "None";
  const statusLabel = t.status || "Completed";
  const refCode = verificationCode(t.termination_no || houseNo || empName);
  const stampDate = effectiveDate.toUpperCase();
  const issuedOn = fmtDate(new Date().toISOString());
...
<head>
<meta charset="UTF-8" />
<title>Clearance Slip – ${esc(t.termination_no)}</title>
<style>
  @import url('https://fonts.googleapis.com/css2?family=Noto+Sans+Ethiopic:wght@400;500;600;700&family=Noto+Serif+Ethiopic:wght@500;700&family=Source+Serif+4:opsz,wght@8..60,400;8..60,600;8..60,700&display=swap');
  * { margin: 0; padding: 0; box-sizing: border-box; }
  @page { size: A4 portrait; margin: 10mm 12mm; }
  ...
  .hdr { width: 100%; border-collapse: collapse; border: 1.5px solid #111; margin-bottom: 0; }
  .hdr td { border: 1px solid #111; padding: 7px 9px; vertical-align: middle; }
  .hdr .logo-cell { width: 12%; text-align: center; }
  .hdr .logo-cell img { width: 54px; height: 54px; object-fit: contain; }
  .hdr .name-cell { width: 56%; text-align: center; }
  .hdr .name-cell .amharic-label { font-size: 8.5pt; color: #475569; font-weight: 500; font-family: "Noto Sans Ethiopic", sans-serif; }
  ...
  .hdr .meta-cell div:last-child { border-bottom: none; }
  .hdr .meta-cell span:first-child { color: #64748b; }
  .title-table { width: 100%; border-collapse: collapse; border: 1.5px solid #111; border-top: none; margin-bottom: 5mm; background: #f8fafc; }
  .title-table td { border: 1px solid #111; padding: 7px 12px; text-align: center; font-weight: 700; font-size: 11pt; font-family: "Noto Sans Ethiopic", sans-serif; }
  .ref-strip { display: flex; justify-content: space-between; font-size: 8.3pt; color: #334155; border-bottom: 1px dashed #cbd5e1; padding-bottom: 4px; margin-bottom: 6mm; font-family: "Noto Sans Ethiopic", sans-serif; }
  .ref-strip b { color: #0a1f33; }
  .body-section { margin-bottom: 7mm; font-family: "Noto Sans Ethiopic", sans-serif; font-size: 10pt; line-height: 2.1; text-align: justify; }
  .signature-section { margin-top: 12mm; width: 100%; display: flex; justify-content: flex-end; font-family: "Noto Sans Ethiopic", sans-serif; }
  .signature-box { text-align: left; width: 260px; font-size: 9.5pt; line-height: 1.5; }
  .signature-box .title { text-transform: uppercase; letter-spacing: 0.6px; color: #64748b; font-size: 7.8pt; font-weight: 700; margin-bottom: 18px; }
  .signature-box .name { font-weight: 700; color: #0a1f33; font-family: "Noto Serif Ethiopic", serif; }
  .signature-box .sign-line { border-top: 1px solid #111; margin-top: 26px; padding-top: 4px; }
  .signature-box .date-field { color: #94a3b8; font-size: 7.8pt; margin-top: 6px; }
  .cc-section { margin-top: 9mm; font-size: 9.3pt; border-top: 1px dashed #cbd5e1; padding-top: 9px; font-family: "Noto Sans Ethiopic", sans-serif; }
  .cc-section .cc-title { font-weight: 700; margin-bottom: 3px; }
  .footer-banner { margin-top: 8mm; border-top: 1.5px solid #111; border-bottom: 1.5px solid #111; text-align: center; padding: 6px; font-size: 8.2pt; font-weight: 700; letter-spacing: 0.6px; text-transform: uppercase; background: #f1f5f9; font-family: "Noto Sans Ethiopic", sans-serif; }
</style>
</head>
<body>
  ...
  ${getOfficialStampHTML(statusLabel.toUpperCase(), stampDate, refCode)}
  ...
  <table class="hdr">
    <tr>
      ...
      <td class="name-cell">
        <div class="amharic-label">የድርጅቱ ስም / Company Name:</div>
        <div class="amharic-name">መተሐራ ስኳር ፋብሪካ</div>
        <div class="english-name">Metahara Sugar Factory</div>
      </td>
      <td class="meta-cell">
        <div><span>Form No.</span><strong>${formNo}</strong></div>
  ...
  <table class="title-table">
    <tr>
      <td>የቤት ርክክብ ማረጋገጫ እና ክሊራንስ ፎርም (Housing Clearance &amp; Handover Slip)</td>
    </tr>
  </table>

  <div class="ref-strip">
    <span>Ref No: <b>${formNo}</b></span>
    <span>House No: <b>${esc(houseNo)}</b></span>
    <span>Status: <b>${esc(statusLabel)}</b></span>
  </div>
  ...
  <div class="body-section">
    ...
    <p>
      ...
      የነበራቸው የሥራ ሁኔታ (${dotVal(caseName, 140)}) በመቋረጡ/በመዘዋወሩ
      የድርጅቱ ቤት ቁጥር ${dotVal(houseNo, 100)} ያስረከቡ መሆናቸውን እናረጋግጣለን።
    </p>
    <br/>
    <p>
      በመሆኑም ሲያስረክብ የተረጋገጠው የጉዳት ግምገማ ድምር ${dotVal(damageCosts, 120)} እና ቀሪ ጉዳዮች
      (${dotVal(outstandingIssues, 180)}) መኖራቸውን/አለመኖራቸውን አረጋግጠን ንብረቱ ጸድቆ ተረክበናል።
    </p>
    <br/>
    <p>
      <strong>ውጤታማ ቀን / Effective Date:</strong> ${dotVal(effectiveDate, 140)}
      &nbsp;&nbsp;
      <strong>ማጣቀሻ / Ref:</strong> ${dotVal(esc(t.termination_no), 140)}
    </p>
  </div>
  ...
  <div class="signature-section">
    <div class="signature-box">
      <div class="title">ከሰላምታ ጋር / Authorized By</div>
      <div class="sign-line">
        <div class="name">ሰቦታ ጀርቤ</div>
        <div class="date-field">የፋ.ሲ.ቲ ማኔጅመንት ቡድኑ መሪ</div>
        <div class="date-field">Date: ______________</div>
      </div>
    </div>
  </div>
  ...
  <div class="cc-section">
    <div class="cc-title">ግልባጭ:-</div>
    <div>ለቤቶች አስተዳደር መድረክ</div>
    <div>Ref No: <strong>${formNo}</strong> &nbsp;|&nbsp; Effective Date: <strong>${effectiveDate}</strong></div>
  </div>
...
export function printClearanceSlip(slip: TerminationTransaction): void {
  const html = buildClearanceSlipHTML(slip);
  const win = window.open("", "_blank");
  if (!win) return;
  win.document.write(html);
  win.document.close();
...
export async function downloadClearanceSlipPdf(slip: TerminationTransaction): Promise<void> {
  const iframe = document.createElement("iframe");
  iframe.style.width = "210mm";
  iframe.style.height = "297mm";
  iframe.style.border = "none";
  ...
  try {
    ...
    iframeDoc.open();
    iframeDoc.write(buildClearanceSlipHTML(slip));
    iframeDoc.close();
    ...
    pdf.addImage(imgData, "JPEG", 0, 0, pdfWidth, pdfHeight);
    const safeRef = (slip.termination_no || "clearance").replace(/[/\\?%*:|"<>]/g, "_");
    const safeName = (slip.employee_name || "employee").replace(/[/\\?%*:|"<>]/g, "_");
    pdf.save(`Clearance_Slip_${safeRef}_${safeName}.pdf`);
  } finally {
    document.body.removeChild(iframe);
  }
}
