...
import {
  listAuditInchargeForUser,
  setAuditInchargeForUser,
} from "@/services/audit";
import {
  listFinalApproverPropsForUser,
  listFinalApproverPropsForEmail,
  setFinalApproverPropsForUser,
  setFinalApproverPropsForEmail,
} from "@/services/finalApprover";
import {
  listUserPropertyAccess,
  setUserPropertyAccess,
} from "@/services/userAccess";
import {
  listUserDepartmentAccess,
  setUserDepartmentAccess,
} from "@/services/userDeptAccess";
import {
  listUserPermissions,
  setUserPermissions,
  type PageKey,
...
export default function Users() {
  ...
  const [userPropertyMap, setUserPropertyMap] = useState<
    Record<string, string[]>
  >({});
  const [authRole, setAuthRole] = useState<string>("");
  const [editSelectedPropertyIds, setEditSelectedPropertyIds] = useState<
    string[]
  >([]);
  const [editInchargePropertyIds, setEditInchargePropertyIds] = useState<
    string[]
  >([]);
  const [editApproverPropertyIds, setEditApproverPropertyIds] = useState<
    string[]
  >([]);
  ...
  const emptyPerms = () =>
    Object.fromEntries(allPages.map((p) => [p, false])) as Record<
      PageKey,
      boolean
    >;
  const [permView, setPermView] =
    useState<Record<PageKey, boolean>>(emptyPerms);
  const [permEdit, setPermEdit] =
    useState<Record<PageKey, boolean>>(emptyPerms);
  const [ePermView, setEPermView] =
    useState<Record<PageKey, boolean>>(emptyPerms);
  const [ePermEdit, setEPermEdit] =
    useState<Record<PageKey, boolean>>(emptyPerms);
  ...
  const filteredUsers = useMemo(() => {
    ...
    return users.filter((user) => {
      ...
      const matchesProperty =
        propertyFilter === "all" ||
        propertyIds.some((pid) => String(pid) === propertyFilter);
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, roleFilter, statusFilter, propertyFilter]);
    ...
    (): ColDef<AppUser>[] => [
      {
        ...
        width: "min-w-[240px]",
        pinned: true,
        value: (user) => `${user.name || ""} ${user.email || ""}`.trim(),
        cell: (user) => (
          <div className="flex items-center gap-3">
            <Avatar className="h-8 w-8">
              {user.avatar_url &&
                !String(user.avatar_url).includes("placeholder") && (
  ...
  const addRequiredMissing = useMemo(
  ...
  useEffect(() => {
    ...
    setPermView(
      Object.fromEntries(allPages.map((p) => [p, defaults[p].v])) as any,
    );
    setPermEdit(
      Object.fromEntries(allPages.map((p) => [p, defaults[p].e])) as any,
    );
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [role]);
  ...
  const handleAddUser = async () => {
    ...
    try {
      ...
      if (password) {
        try {
          await adminSetUserPassword("", "", created.id, password);
        } catch (e) {
          console.error(e);
        }
      }
      ...
      if (selectedPropertyIds.length) {
        try {
          await setUserPropertyAccess(created.id, selectedPropertyIds);
        } catch {}
      }
      // Persist department access mapping
      try {
        await setUserDepartmentAccess(created.id, selectedDepartments);
        if (false) {
      ...
      try {
        if (approverPropertyIds.length) {
          try {
            if (created.email) {
              await setFinalApproverPropsForEmail(
                created.email,
                created.name || null,
                approverPropertyIds,
              );
            } else {
              await setFinalApproverPropsForUser(
                created.id,
                created.name || null,
                approverPropertyIds,
              );
            ...
            toast({
              title: "Final Approver not saved",
              description:
                e?.message || "Could not persist Final Approver assignments.",
              variant: "destructive",
            });
          }
        }
    ...
    setPermView(emptyPerms());
    setPermEdit(emptyPerms());
  };
  ...
  const openEditUser = async (user: AppUser) => {
    ...
    try {
      const pids = await listAuditInchargeForUser(
        user.id,
        user.email || undefined,
      );
      // Normalize returned IDs to match existing property IDs (case-insensitive mapping)
      const byLower = new Map<string, string>(
        properties.map((p) => [String(p.id).toLowerCase(), String(p.id)]),
      );
      const normalized = Array.from(
        new Set(
          pids.map(
            (pid) => byLower.get(String(pid).toLowerCase()) || String(pid),
          ),
        ),
      );
    ...
    try {
      const pids2 = user?.email
        ? await listFinalApproverPropsForEmail(user.email)
        : await listFinalApproverPropsForUser(user.id);
    ...
    try {
      ...
      const merged = mergeDefaultsWithOverrides(
        (user.role || "").toString(),
        perms as any,
      );
      const v: any = Object.fromEntries(
        allPages.map((p) => [p, merged[p]?.v || false]),
      );
      const e: any = Object.fromEntries(
        allPages.map((p) => [p, merged[p]?.e || false]),
      );
      setEPermView(v);
      setEPermEdit(e);
    } catch {}
  ...
  const handleSaveEditUser = async () => {
    ...
    try {
      ...
      if (patch.name && patch.name !== editingUser.name) changes.push("Name");
      if (patch.email && patch.email !== editingUser.email)
        changes.push("Email");
      if (patch.phone && patch.phone !== editingUser.phone)
        changes.push("Phone");
      if (patch.role && patch.role !== editingUser.role) changes.push("Role");
      if (patch.department && patch.department !== editingUser.department)
        changes.push("Department");
      if (patch.status && patch.status !== editingUser.status)
        changes.push("Status");
      await trackActivity("user", "update", {
        entityName: updated?.name || editingUser.name || editingUser.email,
        entityId: editingUser.id,
        changes,
      });
      if (ePassword.trim()) {
        try {
          await adminSetUserPassword("", "", editingUser.id, ePassword.trim());
        } catch (e) {
          console.error(e);
        }
      }
      ...
      // Persist property access mapping
      try {
        await setUserPropertyAccess(editingUser.id, editSelectedPropertyIds);
      } catch {}
      setUserPropertyMap((prev) => ({
        ...prev,
        [editingUser.id]: Array.from(
          new Set(editSelectedPropertyIds.map((id) => String(id))),
        ),
      }));
      // Persist department access mapping
      try {
        await setUserDepartmentAccess(editingUser.id, editSelectedDepartments);
        if (false) {
          console.warn("Department access saved locally only (not remote)");
        }
      } catch {}
      ...
      try {
        const payloadPerms = Object.fromEntries(
          allPages.map((p) => [p, { v: !!ePermView[p], e: !!ePermEdit[p] }]),
        ) as any;
      ...
      try {
        ...
        try {
          ...
          if (editInchargePropertyIds && editInchargePropertyIds.length > 0) {
            await setUserPermissions(editingUser.id, {
              audit: { v: true, e: true },
            } as unknown as Partial<
              Record<PageKey, { v?: boolean; e?: boolean }>
            > as any);
  ...
  const handleDeleteUser = async (userId: string, name: string) => {
    ...
    try {
      ...
      await trackActivity("user", "delete", {
        entityName: name,
        entityId: userId,
      });
      ...
      writeLocalUsers(next);
      toast({
        title: "User deleted (local)",
        description: `${name} removed locally.`,
        variant: "destructive",
      });
    }
  };
  ...
  const handleConfirmResetPassword = async () => {
    ...
    try {
      try {
        await adminSetUserPassword("", "", resetTargetUser.id, newPw);
      } catch (e: any) {
        console.warn(
          "Password reset via service failed, falling back to local.",
          e?.message || e,
        );
      ...
      toast({
        ...
        description: `Password reset for ${resetTargetUser.name}.`,
      });
      ...
      toast({
        title: "Reset failed",
        description: e?.message || "Unable to reset password.",
        variant: "destructive",
      });
    }
  };
            ...
            >
              ...
              {authRole !== "admin" && (
                <div className="mx-6 mt-4 rounded-xl border border-destructive/40 bg-destructive/5 px-4 py-3 text-sm text-destructive-foreground sm:mx-8">
                  You have view-only access. Contact an administrator to add
                  users.
                </div>
              )}
                    ...
                    <div className="grid gap-5 sm:grid-cols-2">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex gap-2">
                          ...
                          <Dialog
                            open={newDeptModalOpen}
                            onOpenChange={setNewDeptModalOpen}
                          >
                            <DialogTrigger asChild>
                              <Button
                                variant="outline"
                  ...
                  <section className="space-y-4">
                    ...
                    <div className="space-y-4">
                      <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          {(
                            ...
                            .map((p) => (
                              <div
                                key={p}
                                className="flex items-center justify-between gap-4 rounded-xl border border-border/60 bg-background p-3 shadow-sm"
                              >
                        ...
                        <p className="text-[10px] text-muted-foreground">
                          Role provides baseline permissions; these are
                          additional per-user overrides.
                        </p>
                      </div>
                        ...
                        <div className="space-y-2">
                          ...
                          <DropdownMenu>
                            ...
                            >
                              ...
                              {deptOptions.filter((d) => d.is_active).length >
                                0 && (
                                <>
                                  <div className="my-1 h-px bg-border" />
                                  <DropdownMenuItem
                            ...
                            <DropdownMenu>
                              ...
                              >
                                {properties
                                  ...
                                  .map((p) => {
                                    const checked =
                                      approverPropertyIds.includes(p.id);
                                    return (
                                      <DropdownMenuItem
                                        key={p.id}
                                        onSelect={(e) => {
                            ...
                            <DropdownMenu>
                              ...
                              >
                                {properties
                                  ...
                                  .map((p) => {
                                    const checked = inchargePropertyIds.some(
                                      (id) =>
              ...
              <DialogFooter className="gap-3 border-t border-border/60 bg-muted/10 px-6 py-5 sm:px-8">
                ...
                <Button
                  ...
                  disabled={
                    authRole !== "admin" || addRequiredMissing || emailInvalid
                  }
      ...
      <Card className="overflow-hidden rounded-2xl border border-border/60 bg-card shadow-sm">
        <CardHeader className="space-y-4 border-b border-slate-300 bg-slate-50 px-4 py-4 dark:border-border dark:bg-muted sm:px-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex items-center gap-2">
              <Badge
                variant="outline"
                className="h-7 rounded-lg border-border/60 bg-background px-3 font-medium"
              >
        ...
        <CardContent className="p-0">
          ...
          <DataTable<AppUser>
            ...
            emptyIcon={<User className="h-10 w-10 text-muted-foreground/50" />}
            exportFileName={`users-${new Date().toISOString().slice(0, 10)}`}
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              ...
              >
                ...
                <div>
                  ...
                  <CardDescription>
                    Manage department visibility, routing, and ownership
                  </CardDescription>
                </div>
              </div>
              {isDepartmentsOpen && (
                    ...
                    onSave={async (name, code) => {
                      ...
                      try {
                        ...
                        toast({
                          title: "Add failed",
                          description:
                            e?.message || "Unable to add department.",
                          variant: "destructive",
                        });
                      }
                    }}
                        ...
                        <TableRow key={d.id}>
                          ...
                          <TableCell className="hidden md:table-cell text-xs text-muted-foreground">
                            {(() => {
                              try {
                                return d.created_at
                                  ? format(new Date(d.created_at), "PP p")
                                  : "—";
                              } catch {
                                return "—";
                              }
                                ...
                                <Dialog>
                                  ...
                                  <DepartmentEditorDialog
                                    ...
                                    onSave={async (name, code) => {
                                      if (isDemoMode()) {
                                        toast({
                                          title: "Blocked in demo",
                                          description:
                                            "Editing departments is disabled in demo.",
                                          variant: "destructive",
                                        });
                                        return;
                                      }
                                  ...
                                  onClick={async () => {
                                    if (isDemoMode()) {
                                      toast({
                                        title: "Blocked in demo",
                                        description:
                                          "Deleting departments is disabled in demo.",
                                        variant: "destructive",
                                      });
                                      return;
                                    }
      ...
      <Dialog open={isResetPwOpen} onOpenChange={setIsResetPwOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {resetTargetUser
                ? `Set a new password for ${resetTargetUser.name}`
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center space-x-2">
                        <Checkbox
                          id="emust"
                          checked={eMustChange}
                          onCheckedChange={(c) => setEMustChange(!!c)}
                          className="h-3.5 w-3.5"
                        />
                        <Label
                          htmlFor="emust"
              ...
              <section className="space-y-4">
                ...
                <div className="space-y-4">
                  <div className="space-y-3">
                    ...
                    <p className="text-[10px] text-muted-foreground">
                      Role provides baseline permissions; these are additional
                      per-user overrides.
                    </p>
                  </div>
                      ...
                      <DropdownMenu>
                        ...
                        >
                          {properties
                            ...
                            .map((p) => {
                              const checked = editSelectedPropertyIds.includes(
                                p.id,
                              );
                              return (
                                <DropdownMenuItem
                      ...
                      <DropdownMenu>
                        ...
                        >
                          {deptOptions
                            .filter((d) => d.is_active)
                            .map((d) => {
                              const checked = editSelectedDepartments.includes(
                                d.name,
                              );
                              return (
                                <DropdownMenuItem
                        ...
                        <DropdownMenu>
                          ...
                          >
                            {properties
                              ...
                              .map((p) => {
                                const checked =
                                  editApproverPropertyIds.includes(p.id);
                                return (
                                  <DropdownMenuItem
                                    key={p.id}
                                    onSelect={(e) => {
                        ...
                        <DropdownMenu>
                          ...
                          >
                            {properties
                              ...
                              .map((p) => {
                                const checked = editInchargePropertyIds.some(
                                  (id) =>
                                    String(id).toLowerCase() ===
                                    String(p.id).toLowerCase(),
                                );
...
function DepartmentEditorDialog({
  ...
  initialName?: string;
  initialCode?: string;
  onSave: (name: string, code?: string) => Promise<void> | void;
}) {
  ...
  return (
    <DialogContent className="max-w-md">
      <DialogHeader>
        <DialogTitle>
          {initialName ? "Edit Department" : "Add Department"}
        </DialogTitle>
        <DialogDescription>
          {initialName
