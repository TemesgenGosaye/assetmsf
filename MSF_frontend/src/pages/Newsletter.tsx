...
export default function Newsletter() {
  ...
  const save = async () => {
    ...
    try {
      if (editing) {
        ...
        await trackActivity("newsletter", "update", { entityName: title.trim(), entityId: editing.id });
      } else {
        ...
        await trackActivity("newsletter", "create", { entityName: title.trim(), entityId: created.id });
      }
      setOpen(false);
      resetForm();
  ...
  const authorInitials = (value?: string | null) => {
    const v = (value || 'SAMS').trim();
  ...
  return (
    <div className="space-y-8 md:space-y-10">
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]">
        <Card className="rounded-2xl border border-border/60 bg-card/95 shadow-lg">
            ...
            <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
              ...
              <div className="flex flex-wrap items-center gap-2">
                {categoryFilters.map((cat) => {
                  ...
                  const hue = isAll ? 'blue' : (categories.find((c) => c.key === cat.key)?.hue || 'blue');
                  ...
                  return (
                    <Button
                      ...
                      className={cn(
                        "h-8 gap-1.5 rounded-lg px-3 text-xs font-semibold transition",
                        active && "border-primary/60 bg-primary/5 text-primary",
                      )}
                      onClick={() => setSelectedCategory(cat.key)}
                    >
                      {!isAll && <span className={cn("h-2 w-2 rounded-full", hueAccent(hue))} />}
                      {cat.label}
                    </Button>
                  );
                })}
          ...
          <CardContent className="space-y-4 px-6 py-6 sm:px-8">
            {loading ? (
              <div className="flex flex-col items-center gap-3 rounded-2xl border border-dashed border-border/60 bg-muted/20 px-4 py-10 text-sm text-muted-foreground">
                <span className="h-8 w-8 animate-spin rounded-full border-2 border-primary/30 border-t-primary" />
                Loading announcements…
              </div>
            ) : filtered.length === 0 ? (
              <div className="flex flex-col items-center gap-2 rounded-2xl border border-dashed border-border/60 bg-muted/20 px-4 py-10 text-center text-sm text-muted-foreground">
              ...
              filtered.map((p, idx) => {
                 ...
                 const catHue = (categories.find((c) => c.key === (p.category || 'release_notes')) || { hue: 'blue' }).hue;
                 return (
                   <article
                     key={p.id}
                     className="group relative overflow-hidden rounded-2xl border border-border/60 bg-background/95 p-0 shadow-sm transition-all hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-lg"
                   >
                     <div className={cn("absolute inset-y-0 left-0 w-1.5", hueAccent(catHue))} />
                     <div className="p-5 pl-6">
                       <div className="flex flex-wrap items-start justify-between gap-3">
                         <div className="flex flex-wrap items-center gap-2 text-xs font-medium">
                           <span className={cn("inline-flex items-center gap-1.5 rounded-lg border px-2.5 py-1 font-semibold", s.cls)}>
                             <span className={cn("h-1.5 w-1.5 rounded-full", hueAccent(catHue))} />
                             {s.label}
                           </span>
                           {!p.published ? (
                             <span className="inline-flex items-center rounded-lg border border-border/60 bg-muted/50 px-2.5 py-1 text-[11px] font-semibold text-muted-foreground">
                               Draft
                             </span>
                           ) : null}
                         </div>
                       ...
                       <h3 className="mt-3 text-lg font-bold leading-snug tracking-tight text-foreground line-clamp-2" title={p.title}>
                         {p.title}
                       </h3>
                       ...
                       <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                         ...
                         <span className="font-medium text-foreground/80">{p.author || 'SAMS Team'}</span>
                       ...
                       <div className="mt-3 text-sm leading-7 text-foreground/90">
                         <div className={cn("relative whitespace-pre-wrap", !expanded && isLong && "max-h-32 overflow-hidden pr-4 md:max-h-44")}>
                           {p.body}
                           {!expanded && isLong && (
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent
          className={cn(
            "!flex max-h-[92vh] w-[calc(100vw-2rem)] flex-col overflow-hidden rounded-2xl border border-border/60 bg-card/95 shadow-xl",
            composeExpanded ? "sm:max-w-4xl" : "sm:max-w-xl",
          )}
        >
          <DialogHeader className="flex flex-row items-center justify-between gap-4 border-b border-border/60 px-6 pb-4 pt-1">
          ...
          <div className={cn("grid min-h-0 flex-1 gap-4 overflow-y-auto px-6 py-4", composeExpanded && "sm:grid-cols-[1.4fr_1fr]")}>
            <div className="grid gap-4">
              ...
              <Textarea
                ...
                className={cn("min-h-[260px] rounded-xl border-border/60 bg-muted/10", composeExpanded && "min-h-[360px]")}
                placeholder={
            ...
            {composeExpanded && (
              <div className="hidden rounded-xl border border-border/60 bg-muted/30 p-4 sm:block">
                <p className="mb-3 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                  Live Preview
                </p>
                <div className="space-y-3">
                  <div className="flex flex-wrap items-center gap-2 text-xs font-medium">
                    <span className={cn("inline-flex items-center gap-1 rounded-md border px-2 py-0.5", statusOf({ published, category } as NewsletterPost).cls)}>
                      {statusOf({ published, category } as NewsletterPost).label}
                    </span>
                    {!published && (
                  ...
                  <p className="whitespace-pre-wrap text-sm leading-6 text-foreground/90">
                    {body.trim() || (
                      <span className="text-muted-foreground">Your update preview will appear here as you type…</span>
                    )}
                  </p>
                </div>
              </div>
          ...
          <DialogFooter className="flex flex-col gap-2 border-t border-border/60 bg-card/95 px-6 py-4 sm:flex-row sm:justify-end">
