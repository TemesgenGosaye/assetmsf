...
export default function Approvals() {
  ...
  const onForward = async (id: string) => {
    try {
      ...
      trackActivity("approval", "update", { entityId: id, oldValue: "pending_manager", newValue: "pending_admin" }).catch(() => {});
  ...
  const onDecision = async (id: string, d: "approved" | "rejected") => {
    ...
    if (res) setItems((s) => s.map(i => i.id === id ? res : i));
    trackActivity("approval", "update", { entityId: id, oldValue: "pending", newValue: d }).catch(() => {});
  };
    ...
    try {
      ...
      if (stage === 'manager') {
        ...
        if (successCount) {
          ...
          await trackActivity("approval", "update", { entityName: `Bulk ${action}`, entityId: successCount, changes: [`${successCount} requests`] });
        }
        if (failureCount) {
      ...
      if (successCount) {
        ...
        await trackActivity("approval", "update", { entityName: `Bulk ${decision}`, entityId: successCount, changes: [`${successCount} requests`] });
      }
      if (failureCount) {
        toast.error(`${failureCount} request${failureCount === 1 ? '' : 's'} failed to update`);
      }
                                    ...
                                    <Button
                                      ...
                                      onClick={async () => {
                                        ...
                                        try {
                                          ...
                                          await trackActivity("approval", "update", { entityName: selectedApproval.id, entityId: selectedApproval.id, changes: ["comment added"] });
            ...
            <Button onClick={async () => {
              ...
              try {
                ...
                if (res) {
                  ...
                  await trackActivity("approval", "update", { entityName: selectedId, entityId: selectedId, changes: ["admin override approved"] });
                }
              } catch (e: any) {
